            ...
            <article>
                    <div class="contenedor">
                        <div class="barra-contenedor">
                            <p>Modificar platillo</p>
                            <a href="platillos.php">Cancelar</a>
                        </div>
                        <div class="informacion" style="background-color: #b0bec5;">
                            ...
                            <div class="formulario-nuevo">
                                <form action="modificar.php?id=<?php echo $platillo['id']; ?>&op=3&nom=<?php echo $platillo['nombre']; ?>" method="post">
                                    <fieldset style="height: 47vh;">
                                        <legend>Datos  platillo</legend>
                                        <input type="text" name="nombre" value="<?php echo $platillo['nombre']; ?>" placeholder="Nombre..." required="true">
                                        <textarea name="descripcion" rows="4" cols="50" placeholder="Descripción"
                                        style="width: 400px;height: 150px;display: block;font-size: 1.2em;margin: 0 auto;padding-left: 10px;padding-right: 10px; margin-top: 20px;border-radius: 10px;resize: none;text-align: justify;font-family: 'Source Sans Pro', sans-serif;"><?php echo $platillo['descripcion']; ?></textarea>
                                        <input type="number" name="precio" value="<?php echo $platillo['precio']; ?>" placeholder="Precio..." required="true">
                                        <select name="estado" id="" disabled="false">
                                        <?php 
                                            switch ($platillo['estado']) {
                                                case 'D':?>
                                            ...
                                            <select name="tipo">
                                                ...
                                                while($contacto= $respuesta->fetch_assoc()){ ?>  
                                                    <?php if($contacto['estado'] == "D"){?>
                                                        <?php if($platillo['tipos_id'] == $contacto['id']){  ?>
                                                            <option value="<?php echo $contacto['tipo']; ?>" selected="selected"><?php echo $contacto['tipo']; ?></option>
                                                        <?php }else{ ?>
